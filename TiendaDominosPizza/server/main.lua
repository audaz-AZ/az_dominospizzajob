...
function LoadShop()
	...
	local shopResult = MySQL.Sync.fetchAll('SELECT * FROM TiendaDominosPizza')
...
ESX.RegisterServerCallback('TiendaDominosPizza:requestDBItems', function(source, cb)
	if not hasSqlRun then
...
RegisterServerEvent('TiendaDominosPizza:buyItem')
AddEventHandler('TiendaDominosPizza:buyItem', function(itemName, amount, zone)
	...
	if amount < 0 then
		print('TiendaDominosPizza: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
